...
export function ApproachTabs() {
  ...
  return (
    <section ref={sectionRef} id="enfoque" className="relative px-6 py-24 md:py-32">
      ...
      <div className="mx-auto max-w-[1280px]">
        <div className="text-center">
          <span className="eyebrow center">{t.approach.eyebrow}</span>
          <h2 className="mt-5 text-[clamp(2rem,4.4vw,3.4rem)] font-semibold">{t.approach.title}</h2>
        </div>

        {/* Barra de pestañas */}
        <div className="mt-10 flex justify-start gap-2 overflow-x-auto pb-1 md:justify-center [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {tabs.map((tb) => (
            <button
              key={tb.id}
              type="button"
              ...
              className={`relative shrink-0 rounded-full px-5 py-2.5 text-[0.95rem] font-medium transition-colors duration-300 ${
                tab === tb.id ? "text-[#04121f]" : "text-muted hover:text-ink"
              }`}
            >
              {tab === tb.id && (
                <motion.span
    ...
    <>
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-2">
        {t.services.items.map((s, i) => (
          <article key={s.title} className="group h-full rounded-2xl glass p-8 transition-[transform,border-color] duration-400 hover:-translate-y-1 hover:border-cyan/40">
    ...
    <>
      ...
      <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {t.sustainability.items.map((it) => (
          <div key={it.title} className="glass h-full rounded-2xl p-7">
            <span className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-cyan/40 text-cyan-bright">
      ...
      <div>
        ...
        <div className="mt-6 space-y-4">
          {t.quality.items.map((q) => (
            <div key={q.k} className="glass rounded-2xl p-5">
              <div className="text-[0.74rem] uppercase tracking-[0.14em] text-cyan">{q.k}</div>
              <div className="mt-1.5 text-[1.02rem] text-ink">{q.v}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        {t.pillars.items.map((p) => (
          <div key={p.n} className="glass rounded-2xl p-6">
            <div className="flex items-baseline gap-3">
              <span className="font-display text-[1.6rem] font-semibold text-cyan-bright">{p.n}</span>
              <h4 className="text-[1.15rem] font-semibold">{p.title}</h4>
            </div>
            <ul className="mt-3 space-y-2">
              {p.points.map((pt, j) => (
                <li key={j} className="flex gap-3 text-[0.94rem] text-muted">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2} className="mt-1 h-[15px] w-[15px] shrink-0 text-cyan"><path d="M5 12l5 5L20 6" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  <span>{pt}</span>
                </li>
              ))}
            </ul>
